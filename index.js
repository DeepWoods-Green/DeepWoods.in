    ...
    try {
        ...
        if (pdfUrl && pdfUrl !== 'general_discussion') {
            ...
            if (retrievedDocs && retrievedDocs.length > 0) {
                const prompt = PromptTemplate.fromTemplate(`Answer the user's question. Use the following context and conversation history to answer.
                ...
                if (searchContext) {
                    const prompt = PromptTemplate.fromTemplate(
                        "You are an AI assistant that answers questions based on the following internet search results. Answer as accurately and concisely as possible. Search Results: {context}\n\nQuestion: {question}"
                    );
